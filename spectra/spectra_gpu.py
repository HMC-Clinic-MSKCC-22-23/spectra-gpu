...
## we don't have to, but could consider reorganizing this to use lightning batching instead
def batchify(to_batch, batch_size):
    """
...
class SPECTRA(pl.LightningModule):  ## change here to inherit from lightning module
    ...
    def loss(self, X, alpha_mask, alpha, batch_size): 
        ...
        eta = 0.5*(eta + eta.T)
        gene_scaling = self.gene_scalings.exp()/(1.0 + self.gene_scalings.exp()) #ctp1 x p
        ...
        gene_scaling_ = contract('ij,ki->jk',gene_scaling, self.factor_to_celltype) #p x L_tot
        ...
        term2 = -1.0*((torch.xlogy(self.adj_matrix*self.weights, (1.0 - rho.reshape(-1,1,1))*(1.0 -kappa.reshape(-1,1,1))*mat + (1.0 - rho.reshape(-1,1,1))*kappa.reshape(-1,1,1)))*self.ct_vec.reshape(-1,1,1)).sum()
        term3 = -1.0*((torch.xlogy(self.adj_matrix_1m,(1.0 -kappa.reshape(-1,1,1))*(1.0 - rho.reshape(-1,1,1))*(1.0 - mat) + rho.reshape(-1,1,1)))*self.ct_vec.reshape(-1,1,1)).sum()
        loss = (self.n/batch_size)*self.lam*term1 + term2 + term3 #upweight local param terms to be correct on expectation 
        return loss
